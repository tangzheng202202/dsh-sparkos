...
test('SQLite migration is idempotent and creates the factory schema', () => {
  ...
  assert.equal(databaseHealth(first, file).schemaVersion, 5)
  first.close()
  const second = openFactoryDatabase({ path: file })
  assert.equal(databaseHealth(second, file).schemaVersion, 5)
  ...
  assert.ok(tables.some((row) => row.name === 'draft_packages'))
  assert.ok(tables.some((row) => row.name === 'draft_artifacts'))
  assert.ok(tables.some((row) => row.name === 'visual_batches'))
  assert.ok(tables.some((row) => row.name === 'visual_asset_tasks'))
  assert.ok(tables.some((row) => row.name === 'visual_asset_attempts'))
  assert.ok(tables.some((row) => row.name === 'visual_asset_events'))
  assert.ok(tables.some((row) => row.name === 'visual_delivery_artifacts'))
  second.close()
