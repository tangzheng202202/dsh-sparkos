...
test('HTTP review/delivery endpoints persist state and page source contains escaped M5B controls without auto-publish', async () => {
  ...
  assert.match(html, /批准图片/); assert.match(html, /驳回图片/); assert.match(html, /按意见重试/); assert.match(html, /测试图片（stub）不可发布/)
  assert.match(html, /esc\(p\.reviewNote\)/); assert.doesNotMatch(html, /<script>reviewXss\(\)<\/script>/)
