...
test('GET /sparkos/app-v2：V2 只读预览版（GET 200 · 内嵌数据 · 仅读无写端点 · POST 404）', async () => {
  ...
  // 受控写边界（M6.3）：V2 允许的写端点为视觉 decision/retry 与草稿 decision/revise，
  ...
  for (const forbidden of ['/sparkos/visual/delivery', '/sparkos/visual/queue', '/sparkos/mutate', '/sparkos/editorial/decision', 'data-editorial']) {
