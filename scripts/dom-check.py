...
if os.path.exists(v2src):
    ...
    check2("v2 controlled-review declaration", "V2 受控操作" in dom2 and "仅开放视觉审核" in dom2)
    # M6.2：V2 允许的写端点扩展为视觉审核 decision 与按意见重试 retry（均仅在人工确认后 POST）
    check2("v2 only controlled visual write endpoints", "/sparkos/visual/decision" in dom2 and "/sparkos/visual/retry" in dom2 and all(x not in dom2 for x in ["/sparkos/creation/decision", "/sparkos/visual/delivery", "/sparkos/visual/queue", "/sparkos/mutate", "/sparkos/editorial/decision", "data-draft-decision", "/sparkos/creation/revise"]))
