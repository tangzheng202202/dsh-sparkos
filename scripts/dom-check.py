...
if os.path.exists(v2src):
    ...
    # M6.3：V2 允许的写端点为视觉 decision/retry 与草稿 decision/revise（均仅在人工确认后 POST）
    check2("v2 only controlled visual and draft write endpoints", "/sparkos/visual/decision" in dom2 and "/sparkos/visual/retry" in dom2 and "/sparkos/creation/decision" in dom2 and "/sparkos/creation/revise" in dom2 and all(x not in dom2 for x in ["/sparkos/visual/delivery", "/sparkos/visual/queue", "/sparkos/mutate", "/sparkos/editorial/decision", "data-editorial"]))
